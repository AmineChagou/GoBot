...
import DialogManagementSystem.GoBot;
import DialogManagementSystem.TestGoBot;
    ...
    public static void mainFunction2(String[] args){
        ...
        for (String s : args) {
            ...
            if (splitArg.length >= 2) {
                ...
                if (option.equals("bot")) botName = value;
                if (option.equals("action")) action = value;
                if (option.equals("domain")) domainFile = value;
                if (option.equals("trace")) {
                    if (value.equals("true")) MagicBooleans.trace_mode = true;
                    else MagicBooleans.trace_mode = false;
                }
        ...
        //Timer timer = new Timer();
        // Create a new GoBot
        domainFile="/Users/aminechagou/Documents/workspace/GoBot/opendial/domains/examples/example-flightbooking.xml";
        GoBot goBot = new GoBot(domainFile,botName, MagicStrings.root_path, action); //

        //if (MagicBooleans.make_verbs_sets_maps) Verbs.makeVerbSetsMaps(bot);
        //bot.preProcessor.normalizeFile("c:/ab/data/log2.txt", "c:/ab/data/log2normal.txt");
        //System.exit(0);
        if (goBot.getBot().brain.getCategories().size() < MagicNumbers.brain_print_size) goBot.getBot().brain.printgraph();
        if (MagicBooleans.trace_mode) System.out.println("Action = '"+action+"'");
        if (action.equals("chat") || action.equals("chat-app")) {
			boolean doWrites = ! action.equals("chat-app");
			TestGoBot.testDialog(goBot, doWrites, MagicBooleans.trace_mode);
		}
        else System.out.println("Unrecognized action "+action);
